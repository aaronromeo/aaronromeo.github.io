# OpenRouter Cost Post — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Write `content/posts/2026-06-20-cost-optimizations-openrouter/index.md` — a full-length, voiced blog post that converts the existing reference playbook into Aaron's personal voice while preserving all technical content verbatim.

**Architecture:** Single-file write. No code, no tests, no build step. The "implementation" is producing the complete `index.md` with correct front matter, voiced prose sections, verbatim technical blocks, inline TODO pun comments, and all accuracy corrections applied. Source `.md` stays untouched.

**Tech Stack:** Hugo blog (Markdown + YAML front matter). No build verification needed — `draft: true` means it won't be served until manually toggled.

**Reference files (read-only):**
- Source playbook: `content/posts/2026-06-20-cost-optimizations-openrouter/openrouter-cost-patterns.md`
- Voice samples: `content/posts/2026-02-23-understanding-emails/index.md`
- Real config: `/Users/aaron/.config/opencode/opencode.json`
- Design spec: `docs/superpowers/specs/2026-06-20-openrouter-cost-post-design.md`
...
| Action | Path | Purpose |
|---|---|---|
| **Create** | `content/posts/2026-06-20-cost-optimizations-openrouter/index.md` | The voiced post |
| **Leave untouched** | `content/posts/2026-06-20-cost-optimizations-openrouter/openrouter-cost-patterns.md` | Reference playbook |

---

## Voice Rules (non-negotiable, check before submitting)

Before marking any task complete, verify the written section contains:

1. At least one **parenthetical aside** per major section (self-deprecating or domestic-metaphor flavored).
2. **Baseball theme** in rendered section headers — light touches only (`lineup`, `bench`, `bullpen`, `small ball`, `starter`, `bullpen`, `blooper`). No forced puns in config/table intros.
3. **Inline TODO HTML comments** on every H2 section header suggesting a punnier baseball alternate: `<!-- TODO: could call this "X" — rationale -->`. These must be *above* or *inline with* the header, invisible when rendered.
4. **Substance intact**: every config block and table reproduced verbatim (character-for-character). No summarizing, no paraphrasing configs.
5. The **deadpan reframe beat** used at least once in the post.
6. At least one **strikethrough joke** (`~~word~~ replacement`).
7. At least one **pull-quote** (`> _text_`) capturing a key principle.

---

## Accuracy Rules (non-negotiable, check before submitting)

Apply every correction from the design spec:

**Fusion:**
- Panel = **1–8 models**, default Quality preset = **3**. Never say "3–6."
- Judge **compares/analyzes**, returns structured JSON (consensus, contradictions, blind spots). **Your model** writes the final answer. Never say "judge synthesizes."
- Cost hits "every request where the model chooses to deliberate" — not "every tool-call cycle."
- "Budget panel" → `general-budget` preset.
- ADD: panel + judge each run web_search/web_fetch loops (cost multiplier).
- ADD: single-level recursion protection (panel/judge cannot re-invoke Fusion).

**Configs:**
- All model IDs: `openrouter/` prefix (e.g. `openrouter/anthropic/claude-sonnet-4.6`).
- `setCacheKey` under `provider.openrouter`, NOT `provider.anthropic`.
- `panel` agent model = `openrouter/openrouter/fusion`.
- §4 drop-in = full real `~/.config/opencode/opencode.json` verbatim (194 lines, full permission block, `instructions`/`plugin` keys included).

**Backstory:**
- Burned **$60 in a single morning**. ✓
- Stack = "an observability stack" or "an observability rabbit hole." ✓
- **No repo name, no PR link.** ✓

---

## Task 1: Front Matter + Title Comment Block
...
- [ ] **Step 2: Verify file exists and front matter parses cleanly**

  Read the file back and confirm: no YAML syntax errors, `draft: true` present, all 5 tags present.

---

## Task 2: Cold Open + TL;DR Stack

**Files:**
- Modify: `content/posts/2026-06-20-cost-optimizations-openrouter/index.md`

This is the hook. The $60 backstory, the "here's the lineup" pivot, and the recommended-stack table verbatim. This section must land the frugal-dad voice immediately.

- [ ] **Step 1: Write the cold open prose**
...
- [ ] **Step 2: Write the TL;DR section header and table**
...
- [ ] **Step 3: Read back and verify**
  ...
  Read the file. Confirm: pull-quote is present, TL;DR table has 6 rows, all model IDs have `openrouter/` prefix, $60 figure appears.

---

## Task 3: §1 — Why Fusion Isn't Your Everyday Starter

**Files:**
- Modify: `content/posts/2026-06-20-cost-optimizations-openrouter/index.md`

This section explains *how* Fusion works (accurately) and when it's the wrong tool. The key accuracy corrections live here.

- [ ] **Step 1: Write §1 with corrected Fusion description**
...
- [ ] **Step 2: Read back and verify**
  ...
  Confirm: "up to 8 models" present, "judge compares" (not "synthesizes") present, "every request where your model chooses to deliberate" present, strikethrough joke present (`~~crutch~~`).

---

## Task 4: §2 — Patterns A and B

**Files:**
- Modify: `content/posts/2026-06-20-cost-optimizations-openrouter/index.md`

- [ ] **Step 1: Write Pattern A — Plan/Execute Split**
...
- [ ] **Step 2: Write Pattern B — Orchestrator + Cheap Subagents**
...
## Task 5: §2 — Patterns C, D, E

**Files:**
- Modify: `content/posts/2026-06-20-cost-optimizations-openrouter/index.md`

- [ ] **Step 1: Write Pattern C — Escalation Ladder**
...
- [ ] **Step 2: Write Pattern D — Aggressive Prompt Caching**
...
- [ ] **Step 3: Write Pattern E — Fusion as a Tool, Not a Default**
...
## Task 6: §2 — Patterns F, G, H

**Files:**
- Modify: `content/posts/2026-06-20-cost-optimizations-openrouter/index.md`

- [ ] **Step 1: Write Pattern F — Context Discipline**
...
- [ ] **Step 2: Write Pattern G — Model-as-a-Specialist**
...
- [ ] **Step 3: Write Pattern H — Variants for Effort Control**
...
## Task 8: §4 — Full Drop-In Config

**Files:**
- Modify: `content/posts/2026-06-20-cost-optimizations-openrouter/index.md`

This is the most important task for accuracy. The config block must be Aaron's actual `~/.config/opencode/opencode.json` reproduced **verbatim** — all 194 lines, including the `instructions`, `plugin`, `permission`, `provider`, `compaction`, and `agent` blocks. Do not trim, summarize, or alter any field.

- [ ] **Step 1: Write §4 intro prose**
...
- [ ] **Step 2: Reproduce the full config verbatim**

  Append the entire contents of `/Users/aaron/.config/opencode/opencode.json` as a `jsonc` code block.

  The block must open with ` ```jsonc ` and close with ` ``` ` and contain exactly:
...
- [ ] **Step 3: Write the daily flow walkthrough**
...
## Task 10: §6 — Quick Reference Tables
...
## Task 12: §8 — Things to Watch + Closing
...
- [ ] **Step 2: Write the closing + further reading**
...
- [ ] **Step 3: Final read-back and checklist**

  Read the completed `index.md` and verify the following before marking complete:

  - [ ] Front matter: `draft: true`, all tags present, baseball-flavored summary
  - [ ] Title TODO comment block present with all 5 alternates
  - [ ] Cold open: $60 figure, anonymized observability stack, no repo/PR link
  - [ ] Pull-quote present
  - [ ] Strikethrough joke present (at least one `~~word~~`)
  - [ ] Deadpan reframe beat present ("Revolutionary" or equivalent)
  - [ ] At least one parenthetical aside per major section
  - [ ] Every H2 has an inline TODO HTML comment with a baseball pun suggestion
  - [ ] Fusion description: "up to 8 models", "judge compares/analyzes", "model writes final answer", "every request where the model chooses to deliberate", "`general-budget` preset"
  - [ ] All model IDs in prose and configs have `openrouter/` prefix
  - [ ] `setCacheKey` under `provider.openrouter` in Pattern D and §4
  - [ ] `panel` agent uses `openrouter/openrouter/fusion`
  - [ ] §4 config block contains `instructions`, `plugin`, full `permission` block
  - [ ] Closing sign-off present ("*Play ball.*")
  - [ ] Source file `openrouter-cost-patterns.md` untouched
...
**Spec coverage check:**
- ✓ Cold open / $60 backstory — Task 2
- ✓ TL;DR table — Task 2
- ✓ §1 Fusion mechanics (corrected) — Task 3
- ✓ Pattern A–H (all 8) — Tasks 4, 5, 6
- ✓ Chat UI §3 — Task 7
- ✓ §4 full real config — Task 8
- ✓ §5 cost math — Task 9
- ✓ §6 quick reference — Task 10
- ✓ §7 checklist — Task 11
- ✓ §8 bloopers + closing — Task 12
- ✓ Voice rules: pull-quote (T2), strikethrough (T3), deadpan (T4), TODO comments (every task)
- ✓ All accuracy corrections applied in relevant tasks
- ✓ Final verification checklist — Task 12, Step 3

**Placeholder scan:** No TBD/TODO text in content steps. All prose blocks contain complete text. ✓

**Type consistency:** N/A (content, not code). ✓
